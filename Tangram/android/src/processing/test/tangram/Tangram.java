package processing.test.tangram;
...
public void loadLevel(int l){switch(l){
    case 0: {
      background(0);
      fill(255); 
      textAlign(CENTER);
...
public void loadShapes(){  for(i=0;i<7;i++){
    pushMatrix();
    translate(px[i],py[i]);
...
public void pickShape(int mColor){
  for(int j=0;j<7;j++){
    if( c[j] == mColor ){
      has = j;
    }
...
public boolean levelFinished(){
  ...
  // Calculate the histogram
    for (int i = 0; i < 1080; i++) {
      for (int j = 0; j < 720; j++) {
        int bright = PApplet.parseInt(brightness(get(i, j)));
        hist[bright]++; 
      }
    }
    if (hist[255]<5000)
...
public void resetShapes(int who){
  if(who < 7 && who >= 0){
    px[who]=PApplet.parseInt(random(250,750));
    py[who]=PApplet.parseInt(random(250,550));
    r[who]=PApplet.parseInt(random(0,8));
    c[who]=color( random(50,200),random(50,200),random(50,200)); //color((255.0/8.0)*(1+i),128,128);
  }
  else{
    for(i=0;i<7;i++){
      px[i]=PApplet.parseInt(random(250,750));
      py[i]=PApplet.parseInt(random(250,550));
      r[i]=PApplet.parseInt(random(0,8));
